...
13. **Latest slice is the last closed one, and it reads in the map's own vocabulary.** Not a bullet list of ids: every touched piece renders **in the same shape its own page uses** — a symbol as a row (glyph, name, path, its one-sentence description pulled live from `LOGIC.md`, the archive's note underneath), a flow as the whole flow case with its steps, because "we changed Sync one group" is unreadable without them. Both carry an **Added / Changed / Removed** pill (word plus colour, never colour alone) and, under the description, an explanation in words, in the pill's own colour — never a number where a sentence belongs. Inside a **changed** flow, the steps the delta note names (`step 4 pull roster`) are highlighted; an **added** flow gets no marks, because the pill already says the whole thing is new. When a changed flow's note names no step, the case says so out loud rather than rendering as if nothing moved.

    The page carries the headline, the captures and the two deltas, and stops there. The plan, the review, the edge paths, the decisions and the diff pulse are in the archive, one click away; reprinting them here turned a document about the system into a second changelog.

    **What the slice *in flight* is changing is not on the board at all** (D-051). It used to be, rendered from the author's uncommitted working tree at generation time — so only the author could ever see it, the published board never could, and it went stale between regenerates. That reading is now `npm run delta`: in the terminal against the working tree while you build, and on the PR, posted by CI from `merge-base...HEAD`, where review actually happens. Same attribution code (`sourcemap.py`), measured against what landed rather than what happens to be dirty.

14. **A capture is evidence, not decoration.** The board is otherwise all prose, so the slice's clips and stills sit near the **top** of *Latest slice* — under the headline, above the symbol rows — full-width, square-edged, hairline-framed, with the archive's one-line caption under each as a micro-label. Two or more ride side by side above 900px and stack below it. A still opens the file on click, through the same `/open` helper a path chip uses; **a clip does not** — clicking a video has to mean play. A slice with no captures renders nothing at all — no placeholder, no "no screenshots" box; the absence is already visible to anyone who expected one.
...
16. **Edge paths are the review, so they render as a review.** A plain full-width row list in the archive's report: surface (rendered as its symbol or flow chip, like every other ref), the state in the middle, what happens as the sentence. No pills, no colour coding by severity — every row is a state that was checked, and ranking them invents a judgement the archive never made. Section omitted entirely when the block is empty.
...
### 4. Latest slice (`#newest`)

The last **closed** slice, out of its archive's `## Report`: headline, captures, symbol delta, flow delta, and the archive's own name as the door to everything else. A slice in flight does not appear here — see rule 13 and `npm run delta`.

Captions come from the archive's `### Shots` block, written at close; the files themselves land at the demo gate.
