...
| L-expenseNew | New expense | Screen | `app/group/[id]/expense/new.tsx` | Records a cost or edits one: who paid (drawer, same shape as currency), how much (large amount field, focused on create), who shares (equal 1-share by default, or share units and fixed cents), and what for last. Edit adds header **Delete** with inline confirm. Edit (`app/group/[id]/expense/[expenseId].tsx`) loads the stored intent. |
...
| L-deleteExpense | `deleteExpense` | Job | `src/sync/groupSync.ts` | Tombstones an expense locally, queues it, and flushes. Missing or already deleted is a no-op. |
