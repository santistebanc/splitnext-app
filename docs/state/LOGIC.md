...
| L-hub | Group hub | Screen | `app/group/[id]/index.tsx` | One group's home: no stack header; Home and Settings sit on the canvas (`L-hubCorner`). Group name centered above the list; names until the first expense (right-aligned, rows expand `L-memberDetail`), then balances as two columns (name right-aligned, amount left-aligned; type grows with leftover space, line boxes 1.4× font so descenders stay visible, capped so a 6-digit amount stays fully visible without scrolling — longer names ellipsize, amounts never do); add member + directly under the list; **Recent activity** (last three events; **Undo** on this device's undoable lines via `L-undoActivity`) pinned above the expense CTAs when anything is recorded — **View all events** expands that block into the full list on this screen (`L-activityFeed`); Settings gear expands `L-settingsPanel`; **View all expenses** expands `L-expensesPanel`; the FAB expands `L-expenseForm`; a toast when sync brings someone else's activity; surfaces the last sync error. |
| L-activity | Activity | Screen | `app/group/[id]/activity.tsx` | Full activity list at `/group/[id]/activity` (URL). The hub path expands `L-activityFeed` in place instead. Scrollable, newest first. The actor's undoable lines show **Undo**. |
| L-hubCorner | `HubCornerChrome` | Screen | `src/ui/HubCornerChrome.tsx` | Home (expands `L-lobbyPanel` on the hub) and Settings on the hub canvas, safe-area inset, no header bar. Settings expands `L-settingsPanel` on the hub. |
| L-activityFeed | `ActivityFeed` | Screen | `src/ui/ActivityFeed.tsx` | Full activity lines with Undo; used by the hub expand and by `L-activity`. |
| L-settings | Settings screen | Screen | `app/group/[id]/settings.tsx` | URL wrapper for `L-settingsPanel`; Close returns to the hub. |
| L-settingsPanel | `SettingsPanel` | Screen | `src/ui/SettingsPanel.tsx` | Group name and a currency picker (same drawer as create), **Close**, Done once the group is named and this device is someone, and Leave group (confirm drawer). Hub expands this from the Settings gear. |
| L-member | Member screen | Screen | `app/group/[id]/member/[memberId].tsx` | URL wrapper for `L-memberDetail`; Close returns to the hub. |
| L-memberDetail | `MemberDetail` | Screen | `src/ui/MemberDetail.tsx` | One member's panel: name + edit in the panel (blur or enter commits), **Close**, invite link if unclaimed, paid-for / owe-for / net / settle once there is an expense, **Remove member** if unclaimed and not You. Hub expands this from a balance row. |
| L-expenses | All expenses | Screen | `app/group/[id]/expenses.tsx` | URL wrapper for `L-expensesPanel`; Close returns to the hub. |
| L-expensesPanel | `ExpensesPanel` | Screen | `src/ui/ExpensesPanel.tsx` | The group's expense list, newest first. A row opens that expense for edit. **Pending** while `L-expenseIsPending`. Hub expands this from **View all expenses**. |
| L-expenseNew | New expense | Screen | `app/group/[id]/expense/new.tsx` | URL wrapper for `L-expenseForm` when creating (or prefilling settle-up). Hub expands the same form from the FAB. |
| L-expenseForm | `ExpenseForm` | Screen | `src/ui/ExpenseForm.tsx` | Records or edits a cost: payer drawer, amount, who shares, what-for; edit adds **Delete** with confirm. **Close** abandons. Hub expands this from the FAB, expense list, member buckets, or settle. |
...
| L-landing | Public landing | Screen | `landing/index.html` | Public GitHub Pages home: showcase copy, a hub preview, **Use on web**, and a footer to privacy and terms. Not the slicer board. |
| L-tryWeb | Use on web | Screen | `landing/try/index.html` | Desktop phone frame with a live iframe of `/app`; a narrow window redirects to full-bleed `/app`. Privacy and terms sit at the bottom-right. Locally `npm run landing` proxies `/app` to Metro. |
| L-inviteLanding | Invite landing | Screen | `landing/404.html` | Public `/j/{token}` page: landing chrome, a continue control into `/app/j/{token}` on desktop, app-directed copy on a phone (no store listing). Other unknown paths show a generic not-found. Footer links privacy and terms. |
| L-privacy | Privacy | Screen | `landing/privacy/index.html` | Public privacy page: no accounts, capability tokens, what is stored, deletion-on-request without a join-code wipe. |
| L-terms | Terms | Screen | `landing/terms/index.html` | Public terms: records and suggests, never moves money, no accounts. |
...
| L-activityRow | `ActivityRow` | Screen | `src/ui/ActivityRow.tsx` | One activity line plus a relative time label on the right; the label refreshes every minute. An **Undo** control when the line can be undone. |
| L-activityToast | `ActivityToast` | Screen | `src/ui/ActivityToast.tsx` | Brief banner on the hub when sync brings someone else's activity; auto-dismiss; tap expands activity on the hub. |
...
| L-parseInviteToken | `parseInviteToken` / `joinPathForToken` | Pure | `src/domain/invite.ts` | Pulls the invite secret out of a raw token, a `/j/{token}` path, or a `/join?token=` URL; builds the public `/j/{token}` path to copy (Pages prefix, not `/app`). |
| L-invitePath | `invite_token_from_path` / `is_invite_landing_path` | Pure | `docs/scripts/invite_landing.py` | Whether a public-site path is an invite (`/j/{token}`, including the Pages prefix) and the 11-char secret if it is. Expo `/app/j/…` is not this page. |
...
| L-openGroup | `openGroup` | Job | `src/sync/groupSync.ts` | Opens a group for viewing — remembers it as last opened, registers for push when `L-notificationsAvailable`, subscribes for wakes, then runs a full sync. The wake socket being down does not block opening. |
...
| L-deleteExpense | `deleteExpense` | Job | `src/sync/groupSync.ts` | Tombstones an expense locally, queues it and an `expense_deleted` activity with `undo_snapshot` when this device has an assumed member, and flushes. Missing or already deleted is a no-op. |
| L-deleteMember | `deleteMember` | Job | `src/sync/groupSync.ts` | Tombstones a member locally, queues it and a `member_kicked` activity with `undo_snapshot` when this device has an assumed member, and flushes. Missing or already deleted is a no-op. |
| L-planUndo | `planUndo` / `activityCanUndo` | Pure | `src/domain/undo.ts` | From an activity plus the current expense or member, the next entity and a tombstoned activity, or null: actor-only, snapshot required, add refuses if the expense was edited after. |
| L-undoActivity | `undoActivity` | Job | `src/sync/groupSync.ts` | Applies that plan, queues the entity then the tombstoned activity, and flushes. Missing activity or a refused plan is a no-op. |
| L-activityForExpenseAdded | `activityForExpenseAdded` | Pure | `src/domain/activity.ts` | Builds a version-1 `expense_added` activity for an expense, or null when ids are missing. Optional `undo_snapshot` is the expense as written. |
...
| L-activityForExpenseDeleted | `activityForExpenseDeleted` | Pure | `src/domain/activity.ts` | Builds a version-1 `expense_deleted` activity, or null when ids are missing. Optional `undo_snapshot` is the live expense before the tombstone. |
| L-activityForMemberKicked | `activityForMemberKicked` | Pure | `src/domain/activity.ts` | Builds a version-1 `member_kicked` activity, or null when ids are missing. Optional `undo_snapshot` is the live member before the kick. |
...
| L-formatActivityLine | `formatActivityLine` / `sortActivities` / `formatActivityLinePlain` | Pure | `src/domain/activity.ts` | Turns an activity plus members and expenses into one readable line per kind; plain text for push. Sorts live events newest first. Tombstoned targets still format for delete/kick lines. `canUndo` is true only when `L-planUndo` would succeed. |
...
| L-expenseIsPending | `expenseIsPending` | Pure | `src/sync/queuePolicy.ts` | True while this expense id is still in the outbound queue, any version. |
...
| L-notificationsAvailable | `notificationsAvailable` | Pure | `src/push/notificationsAvailable.ts` | False in Expo Go (`appOwnership === 'expo'`). True in a development or store build. Callers must not import `expo-notifications` when this is false. |
| L-registerPushToken | `registerPushTokenForGroup` | Job | `src/push/registerPushToken.ts` | Registers this device's Expo push token with the Worker for a group. No-op on web and in Expo Go; dynamic-imports `expo-notifications` only when available. |
...
| L-usePushNotificationOpen | `usePushNotificationOpen` | Job | `src/push/usePushNotificationOpen.ts` | Opens the group hub when the user taps a push notification. Same Expo Go skip as register. |
...
| L-assemblePages | `assemble` | Pure | `docs/scripts/assemble_pages.py` | Builds the public Pages tree: landing at `/` (including privacy, terms, and invite 404), Expo export at `/app`. Never copies the slicer board. |
| L-serveLanding | `metro_path_for` / `prepare_metro_html` | Pure | `docs/scripts/serve_landing.py` | Local landing server: `/try` iframes same-origin `/app`, which proxies Metro; `/j/{token}` serves the invite `404.html`; `/privacy` and `/terms` are static legal pages; HTML is rewritten so Expo Router still sees `/`. |
