...
| --- | --- | --- | --- | --- |
| L-lobby | Lobby screen | Screen | `app/index.tsx` | The first screen (no header): content is vertically centered; **groups** lists every group this device knows by name with a one-line member summary (the section is omitted when there are none), **Create group** (opens the create form), and under it a quiet Join with link (expands to a focused field). Tapping a group opens the hub. |
...
| L-hub | Group hub | Screen | `app/group/[id]/index.tsx` | One group's home: the group name centered in the space above the list (header is home + settings only; home opens the lobby); names until the first expense (right-aligned, rows open member detail), then balances as two columns (name right-aligned, amount left-aligned; type grows with leftover space, capped so a 6-digit amount stays fully visible without scrolling — longer names ellipsize, amounts never do); a discreet add member + under the list on the right; **View all expenses** pinned at the bottom once there is a cost; a FAB to record a cost once bound; surfaces the last sync error. |
| L-settings | Settings screen | Screen | `app/group/[id]/settings.tsx` | Group name and a currency picker (same drawer as create), Done once the group is named and this device is someone, and Leave group (after a confirm). |
| L-member | Member screen | Screen | `app/group/[id]/member/[memberId].tsx` | One member's screen: the display name and an edit icon in the header (tap expands the title to a field; blur or enter commits), an **invite link** heading and join-link row (copy / share) if that slot is unclaimed, and — once the group has an expense — what they paid for and owe for (omitted when empty; a line opens that expense for edit), their net, and suggested settlement as text plus a Settle button. |
| L-expenses | All expenses | Screen | `app/group/[id]/expenses.tsx` | The group's expense list, pushed from the hub, newest first. A row opens that expense for equal-split edit. |
| L-expenseNew | New expense | Screen | `app/group/[id]/expense/new.tsx` | Records a cost or edits one: who paid, how much, what for, and who shares equally among the selected members. Add defaults to You paid and everyone shares, unless the query names a prefill. Edit (`app/group/[id]/expense/[expenseId].tsx`) loads the stored expense. |
...
| L-webFrame | `phoneFrame` | Screen | `src/ui/phoneFrame.ts` | On a wide browser window the app sits in a 420×900 phone frame (`app/+html.tsx`, `src/ui/phoneFrame.web.ts`), always centered, scaled so the whole bezel fits the viewport (shrinks on a narrow or short window, grows on a large one; layout stays 420×900 — a zoom, not a reflow). Drawers and popups portal into that frame (`src/ui/inFrameOverlay.tsx`) instead of covering the desktop. A phone-width window (including capture) stays full-bleed. Native is unchanged (`*.web.ts` is the web half of the import). `src/types/react-dom.d.ts` types `createPortal`. |
...
| L-updateMember | `updateMember` | Job | `src/sync/groupSync.ts` | Renames a member locally and sends the next version to the server. A missing or tombstoned member is a no-op. |
...
| L-patchExpense | `patchExpense` | Pure | `src/domain/expense.ts` | Next version of an expense with a new equal split, or null when the trimmed fields are unchanged, the amount is not positive cents, the payer is not live, or the share set is invalid. |
| L-updateExpense | `updateExpense` | Job | `src/sync/groupSync.ts` | Applies that patch, queues the expense, and flushes. A missing or tombstoned expense is a no-op. |
