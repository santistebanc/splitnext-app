...
| L-serveLanding | `metro_path_for` / `prepare_metro_html` | Pure | `docs/scripts/serve_landing.py` | Local landing server: `/try` iframes same-origin `/app`, which proxies Metro; HTML is rewritten so Expo Router still sees `/`. |
