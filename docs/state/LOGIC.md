...
| L-secureStorage | `getSecret` / `setSecret` / `deleteSecret` | State | `src/secrets/secureStorage.ts` / `src/secrets/secureStorage.web.ts` | The one door to the device's secrets. Native goes to the OS keychain; the web build swaps in `localStorage`, which is why nothing wider than a per-group token belongs here. |
...
| L-efLeave | `handleLeave` | Endpoint | `workers/src/index.ts` | Revokes this device's access token after a capability check. Already-revoked is still success. The member and expenses are not touched. |
