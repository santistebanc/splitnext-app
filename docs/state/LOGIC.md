...
| L-tryWeb | Use on web | Screen | `landing/try/index.html` | Desktop phone frame with a live iframe of `/app`; a narrow window redirects to full-bleed `/app`. Locally `npm run landing` proxies `/app` to Metro. |
| L-inviteLanding | Invite landing | Screen | `landing/404.html` | Public `/j/{token}` page: landing chrome, a continue control into `/app/j/{token}` on desktop, app-directed copy on a phone (no store listing). Other unknown paths show a generic not-found. |
...
| L-serveLanding | `metro_path_for` / `prepare_metro_html` | Pure | `docs/scripts/serve_landing.py` | Local landing server: `/try` iframes same-origin `/app`, which proxies Metro; `/j/{token}` serves the invite `404.html`; HTML is rewritten so Expo Router still sees `/`. |
