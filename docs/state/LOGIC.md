...
| L-activityLineText | `ActivityLineText` | Screen | `src/ui/ActivityLineText.tsx` | Renders one activity line with the expense description in semibold ink so it stands out from the rest of the sentence. |
...
| L-formatActivityLine | `formatActivityLine` / `sortActivities` | Pure | `src/domain/activity.ts` | Turns an activity plus members and expenses into one readable line; sorts live events newest first. |
