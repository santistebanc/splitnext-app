...
**Destination** — A mobile app for splitting shared costs among a small group of friends: groups, members, expenses, derived balances, and settle-up suggestions. Records and suggests; never moves money. The hub is becoming the shell (headerless; activity and member detail expand in place); other screens still stack for now. Public GitHub Pages is a landing plus framed **Use on web**; `/j/{token}` is an invite page; privacy and terms sit on the same host. The slicer board is local-only. Not in destination: create-with-full-roster, group-wide invite, short join code, changing who I am.
