...
- Repo — `github.com/santistebanc/splitnext-app`, public; a slice is branch → PR (CI: `npm test` + `npm run typecheck` + `npm run test:board`) → squash merge → tag `slice-NNNN` on `main` — D-028
