...
- Wake channel — Realtime broadcast on `group:{id}`; payload is tip only; `rt-jwt` gates subscribe (anon_channel fallback — D-006); a drop then `SUBSCRIBED` runs the same catch-up as open (D-054)
- Hosting — remote `splitnext-v3` — D-001
- Server deploy — `.github/workflows/supabase.yml` on every merge to `main`: `db push`, stamp `DEPLOY_SHA`, deploy every function in `FUNCTIONS`, verify each health endpoint; never by hand — D-052, D-053
...
- `shouldCatchUpOnStatus` — `src/sync/wakePolicy.ts` — vitest — whether a Realtime status change means this group missed wakes
