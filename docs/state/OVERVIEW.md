...
Last updated: slice 0056

## Direction

**Destination** — A mobile app for splitting shared costs among a small group of friends: groups, members, expenses, derived balances, and settle-up suggestions. Records and suggests; never moves money. The hub is becoming the shell (headerless; activity and member detail expand in place); hub-as-shell overlays are complete. Public GitHub Pages is a landing plus framed **Use on web**; `/j/{token}` is an invite page; privacy and terms sit on the same host. The slicer board is local-only. Not in destination: create-with-full-roster, group-wide invite, short join code, changing who I am.
...
- Open a group hub: no stack header — Home and Settings sit on the canvas; group name as large centered type above the list; names until the first expense, then balances (You highlighted; tap a name expands member detail on the hub); roster line boxes leave room for descenders; add member + directly under the list; **Recent activity** (last three, with relative times) pinned above the expense CTAs once anything is recorded — **View all events** expands that list on the hub; a top toast when someone else mutates the group while this hub is open; **View all expenses** at the bottom once spent; Home opens the lobby switcher in place; settings gear expands group settings in place (name, currency, Done, Leave, Close); **View all expenses** expands the list in place; FAB expands the expense form in place once bound — [slice 0001](slices/0001-walking-skeleton.md) / [slice 0023](slices/0023-member-first-hub-chrome.md) / [slice 0027](slices/0027-first-run.md) / [slice 0028](slices/0028-member-rename.md) / [slice 0034](slices/0034-activity-spine.md) / [slice 0035](slices/0035-activity-toast.md) / [slice 0036](slices/0036-activity-event-kinds.md) / [slice 0045](slices/0045-hub-name-descenders.md) / [slice 0046](slices/0046-hub-shell-activity.md) / [slice 0047](slices/0047-member-accordion.md) / [slice 0048](slices/0048-settings-overlay.md) / [slice 0049](slices/0049-all-expenses-overlay.md) / [slice 0050](slices/0050-expense-form-overlay.md) / [slice 0051](slices/0051-lobby-switcher.md) / [slice 0050](slices/0050-expense-form-overlay.md)
...
- Receive Expo push when another member's activity merges while this device is backgrounded (development or store build); tap opens the group hub. Expo Go and web skip registration so the native module is never imported — [slice 0038](slices/0038-push-notifications.md) / [slice 0043](slices/0043-expo-go-push-skip.md)
...
- Tap a settle button to expand the expense form on the hub already filled for that transfer; saving records it and returns to member detail; the tap does not move money — [slice 0019](slices/0019-settle-prefill.md) / [slice 0023](slices/0023-member-first-hub-chrome.md) / [slice 0050](slices/0050-expense-form-overlay.md)
...
| `/group/[id]` | Hub: no stack header (Home + Settings on the canvas); group name as large centered type above the list; names until the first expense (rows expand member detail in place; no expense list link), then balances (You highlighted; tap expands member detail); add member + directly under the list; **Recent activity** (last three, relative times; **Undo** on this device's undoable lines) above the expense CTAs once anything is recorded — **View all events** expands the full list here; top toast when sync brings someone else's activity; **View all expenses** expands the in-place expense list; settings gear expands the in-place settings panel; FAB expands `L-expenseForm` once bound; expense rows and settle prefill expand the same form; typed sync error; open → syncGroup | slice 0001–0007 / 0012 / 0017 / 0018 / 0019 / 0023 / 0027 / 0028 / 0034 / 0035 / 0036 / 0045 / 0046 / 0047 / 0048 / 0049 / 0050 / 0056 |
| `/group/[id]/activity` | Full activity list with relative timestamps, newest first; **Undo** on this device's add/delete/kick lines. Hub opens this as an in-place expand; this route remains for the URL | slice 0034 / 0035 / 0036 / 0040 / 0046 / 0056 |
| `/group/[id]/settings` | Group name and currency; Done once named and bound; Leave group (confirm drawer). Hub settings gear expands this same panel in place; this route remains for the URL wrapper | slice 0027 / 0032 / 0048 |
| `/group/[id]/member/[memberId]` | Member: URL wrapper for `L-memberDetail` (name + edit in the panel, **Close**); join link + copy/share if unclaimed; Remove member (confirm drawer) if unclaimed and not You; paid-for / owe-for / net / suggested settlement once the group has an expense; a bucket line expands `L-expenseForm` for edit; Settle expands prefilled `L-expenseForm`. Hub expands the same panel in place | slice 0023 / 0024 / 0025 / 0027 / 0028 / 0029 / 0033 / 0047 / 0050 |
| `/group/[id]/expenses` | URL wrapper for the expense list; Close returns to the hub | slice 0023 / 0029 / 0049 |
| `/group/[id]/expense/new` | URL wrapper for `L-expenseForm` when creating (or prefilling settle-up). Hub expands the same form from the FAB | slice 0018 / 0019 / 0030 / 0031 / 0032 / 0050 |
| `/group/[id]/expense/[expenseId]` | URL wrapper for `L-expenseForm` when editing; hub expands the same form from expense rows and member buckets | slice 0029 / 0050 |
...
- `notificationsAvailable` — `src/push/notificationsAvailable.ts` — vitest — whether this runtime may import `expo-notifications` (false in Expo Go)
