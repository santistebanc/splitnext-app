...
| D-031 | The web app deploys to the same GitHub Pages site under `/app`, not to a separate host | post-0007 | No second account and no deploy token beyond the `EXPO_PUBLIC_` values the client ships anyway; the board's "Open the app" link stays same-origin. The trade is no per-PR preview URL — a Vercel or Cloudflare project can be added if that ever earns its keep. |
| D-032 | Board path chips point at GitHub `blob/main`, and only a board served from localhost opens the editor | post-0007 | The published board has readers with no clone; the editor route was the only one before, which made every chip dead for them. |
