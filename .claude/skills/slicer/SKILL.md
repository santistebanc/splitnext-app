...
The current slice is **not** on the board. Locally it is `/slice.html` (`npm run board -- --slice-page`); CI generates the same page per PR and comments the URL (D-055). `npm run delta` is the same attribution as text.

The board is **local-only** (`npm run board:serve` → `http://127.0.0.1:8777/slicer.html`). Production Pages is the landing at `/` plus the app at `/app` (D-093). Do not copy `slicer.html` into the Pages tree. Path chips open the editor only on the local server.
...
Merging `main` republishes the public landing at `/` and the web app at `/app` on GitHub Pages. The slicer board is local-only (`npm run board:serve`) and must not become the production homepage (D-093). The slice page for a PR is generated by CI on that PR (D-055), not by the merge.
...
**`docs/slicer.html` and `docs/slice.html` are not committed.** They are renders of `docs/state/`, so a tracked copy only ever produced rebase conflicts and stale boards. They are gitignored and rebuilt locally by `npm run board` / `npm run board:serve`. CI still generates the per-PR slice page (`--slice-page`) onto the `pr-boards` branch.
