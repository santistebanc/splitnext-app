...
8. **A path is a door — opened by the CLI, not the URL scheme.** Every file path links to its declaration line, found at generation time, and the chip shows `path:line`. Serve the board with `docs/scripts/serve-slicer-board.py`; clicking a path then fetches `/open?path=…&line=…` and the helper runs **`cursor -r -g <file>:<line>`**, which reuses the window already attached to this folder. This is the only reliable route: a `cursor://` URL goes through the OS handler and, inside WSL, opens a *second* instance, and the IDE's built-in browser blocks custom schemes entirely — the fetch works in both. The `cursor://` href stays as the fallback for opening the file straight off disk. The helper only opens paths inside the repo.
